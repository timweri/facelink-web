  ...
  inquirer.prompt([
    {
      ...
      choices: [
                { name: 'Quickly (Just enter title and we generate the rest for you)', value: 1 },
                { name: 'Advanced (Open text editor to enter json array list)', value: 2 },
                { name: 'Back', value: 3 }
      ]
    }
  ]).then(result => {
    if (result.type === 1) { getUserInput(cb) } else if (result.type === 2) {
      inquirer.prompt([
        {
          type: 'editor',
          name: 'jsonArray',
        ...
        if (arr.length < 1) { askForInputType(cb) } else { mainConfirmFunc(arr, null, null, false, cb) }
      })
    } else { askForInput(cb) }
  })
}
...
function getFileFromTempInput (str) {
  ...
  if (arr.length > 0) {
    for (var i = 0; i < arr.length; i++) {
      if (typeof (arr[i]) === 'string') { arr[i] = { title: titleize(humanize(arr[i])) } }

      if (arr[i].title) {} else if (arr[i].metaDescription) { arr[i].title = arr[i].metaDescription }

      if (arr[i].metaDescription) {} else if (arr[i].title) { arr[i].metaDescription = arr[i].title }

      if (arr[i].title) {
        if (arr[i].file) {} else if (arr[i].title.length > 1) { arr[i].file = slugify(trimSpace(arr[i].title)) }
      }
    }
    ...
    for (var i = 0; i < arr.length; i++) {
      if (arr[i].title && arr[i].title.length > 1 &&
            arr[i].file && arr[i].file.length > 1) { newarr.push(arr[i]) }
    }
  }
...
function getUserInput (cb) {
  inquirer.prompt({
    ...
    validate: function (value) {
      if (value.length === 0 || trimSpace(value).length === 0) { return 'No files in list. Enter again...' } else { return true }
    }
  }).then(function (value) {
    ...
    for (var i = 0; i < arr.length; i++) {
      ...
      if (arr[i].length > 1) {
        if (arr[i].toLowerCase() !== 'index' && arr[i].toLowerCase() !== 'index.html') {
          newarr.push({ title: titleize(humanize(arr[i])), file: slugify(arr[i]) })
        }
      }
    }
    ...
    for (var i = 0; i < newarr.length; i++) {
      if (newarr[i].file.length > 1 && newarr[i].title.length > 1) { checkarr.push(newarr[i]) }
    }
    ...
    if (checkarr.length === 0) {
      inquirer.prompt({
        ...
        if (result.askAgain === true) { askForInputType(cb) } else { askForInput(cb) }
      })
    } else {
...
async function renderPageModules (filesSetting) {
  ...
  if (pageModules && pageModules.length > 0) { filesSetting = filesSetting.concat(pageModules) }
    ...
    for (var i = 0; i < filesSetting.length; i++) {
      ...
      arr.push(
                gulp.src(pathSettingHtml)
                    ...
                    .pipe(gulp.dest(rootdir, {overwrite: isDeleteIfExists}))
            )

      let dataForStyle = {
        dirname: path.join(config.paths.root.src, 'stylesheets', 'modules'),
        basename: getNameWithoutExt(filesSetting[i].file),
        prefix: '_',
        suffix: '',
        extname: '.scss'
...
async function checkFileData (cb) {
  ...
  if (dataSetting.length > 0) { dataSetting = _.uniqBy(dataSetting, 'file') }
  ...
  if (dataSetting.length > 0) {
    ...
    inquirer.prompt({
      ...
      if (result.askAgain === true) { checkFileData() } else { askForInput(cb) }
    })
  }
}
  ...
  inquirer.prompt([
    {
      ...
      choices: [
                { name: 'Direct input separate list of pages', value: 1 },
                { name: 'Files list in ' + path.resolve(baseDirScript, pageList), value: 2 },
                { name: 'Exit', value: 3 }
      ]
    }
  ]).then(answer => {
...
function mainConfirmFunc (arrPages, additionModules, oldarr, deleteIfExists, cb) {
  ...
  if (additionModules && additionModules.length > 0) { pageModules = pageModules.concat(additionModules) }
  ...
  if (arrPages && arrPages.length > 0) {
    ...
    for (var i = 0; i < temparrPages.length; i++) {
      if (temparrPages[i].file.toLowerCase().indexOf('.html') < 0) { temparrPages[i].file += '.html' }
    }
  }
  ...
  if (additionModules && additionModules.length > 0) {
    ...
    for (var i = 0; i < temparrModules.length; i++) {
      if (temparrModules[i].file.toLowerCase().indexOf('.html') < 0) { temparrModules[i].file += '.html' }
    }
  }
